  ...
  template: `
    <app-layout>
      <div class="max-w-7xl mx-auto space-y-8 px-4 py-8">
        ...
        <h1 class="text-3xl font-bold border-b border-white/10 pb-4">Mes Favoris</h1>

        <div *ngIf="favorites$ | async as favorites">
          <!-- Empty State -->
          <div *ngIf="favorites.length === 0" class="flex flex-col items-center justify-center py-20 text-center text-gray-400">
             <div class="bg-white/5 p-6 rounded-full mb-4">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                </svg>
             </div>
             <p class="text-xl font-medium">Aucun favori pour le moment.</p>
             <p class="mt-2 text-sm">Explorez les films et cliquez sur "Ajouter aux favoris" !</p>
             <a routerLink="/" class="mt-6 px-6 py-2 bg-blue-600 hover:bg-blue-700 rounded-full text-white transition">Découvrir des films</a>
          </div>

          <!-- Grid -->
          <div *ngIf="favorites.length > 0" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            <a *ngFor="let movie of favorites" [routerLink]="['/movie', movie.id]" class="group relative bg-[#1a1d26] rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition hover:-translate-y-1 block">
               <div class="aspect-[2/3] relative">
                  ...
                  <button 
                    (click)="removeFavorite($event, movie.id)" 
...
export class FavoritesComponent {
  ...
  favorites$: Observable<Movie[]> = this.authService.getFavorites();
  removeFavorite(event: Event, movieId: number) {
    event.stopPropagation();
    event.preventDefault();
    if (confirm('Voulez-vous vraiment retirer ce film des favoris ?')) {
      this.authService.removeFavorite(movieId).subscribe();
    }
  }
}
